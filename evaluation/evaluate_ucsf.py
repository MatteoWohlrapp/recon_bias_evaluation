...
def fairness_prediction_classifier(predictions):
    ...
    for prediction in predictions:
        ...
        if prediction['fairness']:
            ...
            prediction_results['age_bin'] = prediction_results['age'].apply(lambda x: "Y" if float(str(x).split('(')[1].split(',')[0]) <= 58 else "O")
            prediction_results['sex'] = prediction_results['sex'].apply(lambda x: "M" if float(str(x).split('(')[1].split(')')[0]) == 1 else "F")
            prediction_results['cns'] = prediction_results['cns'].apply(lambda x: float(str(x).split('(')[1].split(')')[0]))
            prediction_results['diagnosis'] = prediction_results['diagnosis'].apply(lambda x: float(str(x).split('(')[1].split(')')[0]))
            ...
            for interpreter, (interpreter_name, gt_name) in interpreters.items():
                ...
                baseline_threshold = calculate_threshold(patient_preds_class[f'{interpreter_name}'], patient_gt[gt_name])
                recon_threshold = calculate_threshold(patient_preds_recon[f'{interpreter_name}_recon'], patient_gt[gt_name])

                patient_preds_class[f'{interpreter_name}'] = patient_preds_class[f'{interpreter_name}'].apply(lambda x: 1 if x >= baseline_threshold else 0)
                patient_preds_recon[f'{interpreter_name}_recon'] = patient_preds_recon[f'{interpreter_name}_recon'].apply(lambda x: 1 if x >= recon_threshold else 0)
                ...
                for attribute, (attribute_values, attribute_name) in sensitive_attributes.items():
                    ...
                    for attribute_value in attribute_values:
                        ...
                        filtered_preds_class = patient_preds_class[patient_preds_class[attribute] == attribute_value]
                        filtered_preds_recon = patient_preds_recon[patient_preds_recon[attribute] == attribute_value]
                        filtered_gt = patient_gt[patient_gt[attribute] == attribute_value]

                        tpr_class.append(get_tpr(filtered_preds_class[f'{interpreter_name}'], filtered_gt[gt_name]))
                        fpr_class.append(get_fpr(filtered_preds_class[f'{interpreter_name}'], filtered_gt[gt_name]))
                        tpr_recon.append(get_tpr(filtered_preds_recon[f'{interpreter_name}_recon'], filtered_gt[gt_name]))
                        fpr_recon.append(get_fpr(filtered_preds_recon[f'{interpreter_name}_recon'], filtered_gt[gt_name]))
    ...
    for prediction in predictions:
        ...
        for attribute, (attribute_values, attribute_name) in sensitive_attributes.items():
            ...
            for attribute_value in attribute_values:
                ...
                dice_recon_by_group.append(filtered_preds_recon['UNet_recon_dice'].mean())
...
def evaluate_ucsf(config, results_dir, name):
    ...
    fairness_results = pd.concat([fairness_results, fairness_prediction_segmentation(predictions)])
