...
def plot_chex_performance(results, results_dir, name):
    ...
    metrics = ["ec", "cardiomegaly", "lung-opacity", "lung-lesion", "edema", "consolidation", "pneumonia", "atelectasis", "pneumothorax", "pleural-effusion", "pleural-other", "fracture", "average"]
    ...
    for metric in metrics:
        ...
        data1 = results[(results['metric'] == metric) & (results['model'] != 'baseline')].copy()
        data2 = results[(results['metric'] == "psnr") & (results['model'] != 'baseline')].copy()
        ...
        custom_lines = [
            Line2D([0], [0], color='gray', linestyle='-', marker='o', 
                  label="AUROC", linewidth=2.5, markersize=8),
            Line2D([0], [0], color='gray', linestyle='--', marker='s', 
                  label="PSNR", linewidth=2.5, markersize=8)
        ]
        ...
        for fmt in ['eps', 'pdf', 'png']:
            plt.savefig(
                f'{results_dir}/performance_plots/{name}_performance_{metric}_psnr.{fmt}', 
                bbox_inches='tight', 
                dpi=300,
                format=fmt
            )
...
def plot_chex_additional_bias(results, results_dir, name):
    ...
    interpreters = ["ec", "cardiomegaly", "lung-opacity", "lung-lesion", "edema", "consolidation", "pneumonia", "atelectasis", "pneumothorax", "pleural-effusion", "pleural-other", "fracture", "average"]
    ...
    for interpreter in interpreters:
        ...
        for metric in interpreter_configs['metrics']:
            ...
            metric_data['metric_type'] = interpreter_configs['x_labels'][interpreter_configs['metrics'].index(metric)]
        ...
        col_names = {"gender": "Gender", "age": "Age", "ethnicity": "Ethnicity"}
